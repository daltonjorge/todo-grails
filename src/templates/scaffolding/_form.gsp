...
private renderFieldForProperty(p, owningClass, prefix = "") {
  ...
  if (hasHibernate) {
    ...
    isBooleanType = cp.propertyType in [boolean, Boolean]
    required = (cp ? !isBooleanType && !cp.nullable && (cp.propertyType != String || !cp.blank) : false)
  }
  if (display) { %>
<div class="form-group \${hasErrors(bean: ${propertyName}, field: '${prefix}${p.name}', 'error')} ${required ? 'required' : ''}">
  ...
  <% } else { %>
  <label class="col-sm-2 control-label" for="${prefix}${p.name}">
    <g:message code="${domainClass.propertyName}.${prefix}${p.name}.label" default="${p.naturalName}" />
